...
def MainMenu():
	addDir('[COLOR gold]Instalar Wizard PECASCUCO NEXUS [/COLOR]','',1,addon_icon,addon_fanart,local_string(30001),isFolder=True)
...
def main(NAME, NAME2, VERSION, URL, ICON, FANART, DESCRIPTION):
	...
	yesInstall = dialog.yesno(NAME, 'instalar Wizard PECASCUCO para Nexus', nolabel='Cancelar', yeslabel='Continuar')
	if yesInstall:
	    save_check()
	    save_move1()
...
def freshStart():
	...
	if yesFresh:
		...
		if mode == 4:
			...
			dialog.ok(addon_name, 'PECASCUCO .')
			os._exit(1)
	else:
		return
...
def buildInstall(NAME, NAME2, VERSION, URL):
	...
	addon.setSetting('firstrun', 'true')
	dialog.ok(addon_name, 'DEBES DE INSTALAR ACESTREAM PARA VER LOS DEPORTES.AHORA FUERZA EL CIERRE DE KODI PARA TERMINAR.')
	os._exit(1)
